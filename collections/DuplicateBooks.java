        ...
        for (int i = 0; i < library.size(); i++) {
            for (int j = i + 1; j < library.size(); j++) {
                if (library.get(i).equals(library.get(j))) {
                    duplicateBooks.add(library.get(j));

                }
            }
        }
        System.out.println("No of duplicate books are" + duplicateBooks.size());
        duplicateBooks.forEach(System.out::println);
        ArrayList<Book> nonDuplicates = new ArrayList<>();
        for (int i = 0; i < library.size(); i++) {
            if (!duplicateBooks.contains(library.get(i)))
                nonDuplicates.add(library.get(i));
